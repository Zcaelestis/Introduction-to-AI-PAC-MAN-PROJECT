...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        self.visitedCorner = [False, False, False, False]  # action
    ...
    def getStartState(self):
        ...
        startState = (self.startingPosition, tuple(self.visitedCorner))
    ...
    def isGoalState(self, state):
        ...
        # util.raiseNotDefined()
        # print(state)
        return all(corner != False for corner in state[1])
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                if nextState in self.corners:
                    visitedCorner[self.corners.index(nextState)] = True

                successors.append(
                    ((nextState, tuple(visitedCorner)), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    for i in range(len(Corners)):
        if Corners[i] == True:
            print(Corners[i])
            visitedCorner.append(corners[i])
    # print(visitedCorner)
    lowerBound = 0
    count = len(corners) - len(visitedCorner)
    print(corners)
    print(visitedCorner)
    while count > 0:
        distance = []
        ...
        for i, corner in enumerate(corners):
            if corner not in visitedCorner:
                distance.append(
                    (util.manhattanDistance(corner, currentNode), corner))
                # print(distance)

        minimumDistance, corner = min(distance)
        lowerBound += minimumDistance
        count -= 1
        currentNode = corner
        visitedCorner.append(corner)
    return lowerBound  # Default to trivial solution
...
def foodHeuristic(state, problem):
    ...
    visitingPosition = state[0]
    foods = foodGrid.asList()
    gameState = problem.startingGameState
    totalDistance = 0
    for foodPosition in foods:
        foodDistance = mazeDistance(visitingPosition, foodPosition, gameState)
        totalDistance = max(totalDistance, foodDistance)
